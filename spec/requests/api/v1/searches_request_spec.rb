...
RSpec.describe "Api::V1::Searches", type: :request do
  ...
  describe '#show' do
    it 'returns the Search object by id' do
      search_1 = create(:search)
      search_2 = Search.create!(query: 'mojito', url: 'https://www.thecocktaildb.com/api/json/v1/1/search.php?s=mojito')
      ...
      expect(results['data'][0]['attributes']['id']).to eq(search_1.id)
      expect(results['data'][0]['attributes']['query']).to eq(search_1.query)
      expect(results['data'][0]['attributes']['url']).to eq(search_1.url)
    end
  end
end
